...
public class LedSettings extends SettingsPreferenceFragment {
	...
	private static final boolean AMLOGIC_PLATFORM = true ;
	private static final boolean ROCKCHIP_PLATFORM = false ;
   ...
   public static class InitLedReceiver extends BroadcastReceiver{
    ...
    private class TriggerListViewAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertview, ViewGroup arg2){
           ...
           holder.radioImage.setImageResource(triggerInfo.isChecked? R.drawable.led_radio_on:R.drawable.led_radio_off);
   ...
   public List getSupportList() {
       ...
       try {
		    FileReader fread;
			if(AMLOGIC_PLATFORM)
            fread = new FileReader(AMLOGIC_RED_TRIGGER_SYSFILE);
			else
			fread = new FileReader(RED_TRIGGER_SYSFILE);
  ...
  private static void BootUpdateLedSettings(){
	 ...
	 int ledNum = 0;
	 if(AMLOGIC_PLATFORM){
        fileRed = new File(AMLOGIC_RED_TRIGGER_SYSFILE);
		ledNum = 1;
	 }else{
        fileRed = new File(RED_TRIGGER_SYSFILE);
		ledNum = 2;
	  }
  ...
  private void updateLedTriggerSettings(TriggerInfo info){
	  ...
	  File file=null;
	   if(info.ledColor.equals(RED_LED)){
		  if(AMLOGIC_PLATFORM)
		  file = new File(AMLOGIC_RED_TRIGGER_SYSFILE);
		  else
